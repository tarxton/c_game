...
// and or not nor xor, no not because its not binary
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) /* runs once at startup, initialization */
{
    ...
    gates[0].y = rects[0].y;
    gates[0].w = 150.0f;
    gates[0].h = 320.0f; //{, rects[0].y, 150.0f, 300.0f};
    ...
    gates[1].y = rects[2].y;
    gates[1].w = 150.0f;
    gates[1].h = 320.0f; //{wires[0].x + wires[0].w, rects[2].y, 150.0f, 300.0f};
    ...
    {
        strcpy(gate_op[i], operations[SDL_rand(5)]);
    }
...
{
    if (event->type == SDL_EVENT_KEY_DOWN && event->key.key == SDLK_Q|| event->type == SDL_EVENT_QUIT) /* quit on key down */
    {
    ...
    if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN)
    {
        for (int i = 0; i < SDL_arraysize(rects); i++)
        {
            if (event->button.x >= rects[i].x && event->button.x <= (rects[i].x + rects[i].w) &&
                event->button.y >= rects[i].y && event->button.y <= (rects[i].y + rects[i].h))
            {
                active[i] = !active[i];
            }
        }
...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    for (int i = 0; i < SDL_arraysize(borders); i++)
    {
        if (active[i])
    ...
    if (gate_active[2]) SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255);
    else SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255);
    SDL_RenderFillRect(renderer, &result_border);

    SDL_SetRenderDrawColor(renderer, 128, 128, 128, 255);
    SDL_RenderFillRects(renderer, wires, SDL_arraysize(wires));
    ...
    for (int i = 0; i < SDL_arraysize(gate_active); i++)
    {
        if (gate_active[i]) SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255);
        else SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255);
